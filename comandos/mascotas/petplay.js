/**
 * name: petplay
 * aliases: ["jugar","playpet"]
 * description: Juega con tu mascota para subir su felicidad y fortalecer el vínculo
 * category: Mascotas
 */
...
export async function run(ctx){
  ...
  if(!qn) return sock.sendMessage(msg.key.remoteJid,{ text:'✨ Usa: $petplay <nombre o id> (o define favorita con $setpet)' },{ quoted: msg })
  ...
  const last = u.petStats[petKey].lastPlay ? new Date(u.petStats[petKey].lastPlay).getTime() : 0
  const now = Date.now()
  if(last && now - last < cdMs){
    const left = Math.ceil((cdMs - (now - last))/60000)
    return sock.sendMessage(msg.key.remoteJid,{ text:`⏳ Ya jugaste recientemente. Espera ${left} min.` },{ quoted: msg })
  }
  if((u.coins||0) < cost){
    return sock.sendMessage(msg.key.remoteJid,{ text:`💔 Te faltan ₭${util.formatKirby(cost - (u.coins||0))} para jugar con tu mascota.` },{ quoted: msg })
  }

  u.coins = (u.coins||0) - cost
  const inc = 12
  u.petStats[petKey].happiness = Math.max(0, Math.min(100, Number(u.petStats[petKey].happiness||0) + inc))
  u.petStats[petKey].lastPlay = nowBogotaISO()
  users[jid] = u
  await saveUsers(files, db, users)

  await sock.sendMessage(msg.key.remoteJid,{ text:`🎾 Jugaste con ${petsDb[petKey].name}. Felicidad: ${u.petStats[petKey].happiness}/100 (₭${util.formatKirby(cost)})` },{ quoted: msg })
}
