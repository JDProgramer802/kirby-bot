...
export async function run(ctx){
  ...
  const v=(args[0]||'').toLowerCase(); if(!['enable','disable'].includes(v)) return sock.sendMessage(gid,{text:'✨ Usa: $welcome enable | disable'},{quoted:msg})
  const groups = await ensureGroupConfig(GROUPS_FILE, db, gid)
  groups[gid].welcome = v==='enable'
  await saveGroups(GROUPS_FILE, db, groups)
  await sock.sendMessage(gid,{text:'🎀 ¡Listo! Bienvenida actualizada 💖'},{quoted:msg})
}
