 ...
 * name: economy
 * aliases: ["economia"]
 * description: Activa/desactiva economía por grupo
...
export async function run(ctx){
  ...
  const v = (args[0]||'').toLowerCase(); if(!['enable','disable'].includes(v)) return sock.sendMessage(gid,{text:'✨ Usa: $economy enable | disable'},{quoted:msg})
  const groups = await ensureGroupConfig(GROUPS_FILE, db, gid)
  groups[gid].modules ||= {}; groups[gid].modules.economy = v==='enable'
  await saveGroups(GROUPS_FILE, db, groups)
  await sock.sendMessage(gid,{text:'🎀 ¡Listo! Economía actualizada 💖'},{quoted:msg})
}
