 ...
 * name: gacha
 * aliases: []
 * description: Activa/desactiva gacha por grupo
...
export async function run(ctx){
  ...
  const v = (args[0]||'').toLowerCase(); if(!['enable','disable'].includes(v)) return sock.sendMessage(gid,{text:'✨ Usa: $gacha enable | disable'},{quoted:msg})
  const groups = await ensureGroupConfig(GROUPS_FILE, db, gid)
  groups[gid].modules ||= {}; groups[gid].modules.gacha = v==='enable'
  await saveGroups(GROUPS_FILE, db, groups)
  await sock.sendMessage(gid,{text:'🎀 ¡Listo! Gacha actualizado 💖'},{quoted:msg})
}
