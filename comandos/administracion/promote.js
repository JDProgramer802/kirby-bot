 ...
 * name: promote
 * aliases: []
 * description: Asciende usuario a administrador
 * category: Administración
 */

import { requireGroup, isAdmin, isBotAdmin, mentionTarget, kirbyCard } from './_common.js'
...
export async function run(ctx){
  ...
  if(!ok){
    const card = kirbyCard({
      ...
      note:'💫 _Usa este poder dentro de tu Dreamland._'
    })
  ...
  if(!rawTarget){
    const card = kirbyCard({
      ...
      quote:'✨ Menciona a quien ascender',
      note:'🌸 _La estrellita guía tus ascensos._'
    })
  ...
  try{
    await sock.groupParticipantsUpdate(gid,[resolved],'promote')
    const gname = meta?.subject || 'Dreamland'
    const card = kirbyCard({
      title:'KIRBY ADMIN',
      lines:[
        '👑 Acción: **Promote**',
        `👤 Usuario: **@${bare(resolved)}**`,
        `📍 Grupo: *${gname}*`
      ],
      quote:'🌸 *¡Listo!* El usuario ahora es admin.',
      note:'💫 _La estrellita brilla más cuando ayuda._'
    })
    ...
    const card = kirbyCard({
      ...
      lines:[
        '👑 Acción: **Promote**',
        `🎯 Objetivo: **@${bare(resolved)}**`
      ],
      quote:`🌸 No pude ascender. ${botIsAdmin? '*Puede ser restricción del grupo o del usuario.*' : '*Parece que no tengo admin.*'}${extra}`,
